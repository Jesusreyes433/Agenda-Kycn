    ...
    <ModalShell title="Acceso de administrador" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <button
          ...
          className="w-full rounded-lg bg-[#0F2540] px-3 py-2.5 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50"
        >
...
export function AdminPanel({ onClose }: Props) {
  ...
  if (!unlocked) {
    return <AdminPasswordGate onUnlock={() => setUnlocked(true)} onClose={onClose} />;
  }

  return <MemberManagement onClose={onClose} />;
}

function MemberManagement({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalShell title="Equipo" onClose={onClose}>
      {loading ? (
